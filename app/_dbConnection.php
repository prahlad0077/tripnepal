...
class Packages extends Database
{
    ...
    public function getPackages($location, $start = 0, $end = 1000)
    {
        ...
        if ($location == "All") {
            ...
            $sql = "SELECT * FROM packages WHERE package_location LIKE '%$location%' OR package_name LIKE '%$location%' OR keywords LIKE '%$location%' ORDER BY package_id DESC LIMIT $start,$end";
        }
    ...
    public function getExclusivePackages($start = 0, $end = 1000)
    {
        $this->connect();
        $sql = "SELECT * FROM packages WHERE is_exclusive = 1 ORDER BY package_id DESC LIMIT $start,$end";
        $result = $this->conn->query($sql);
        $this->conn->close();
        return $result;
    }
    ...
    public function getPackagesWithQueryCount($location)
    {
        ...
        $sql = "SELECT * FROM packages WHERE package_location LIKE '%$location%' OR package_name LIKE '%$location%' OR keywords LIKE '%$location%'";
        $result = $this->conn->query($sql);
        $this->conn->close();
        return $result->num_rows;
    }
